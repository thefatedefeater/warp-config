...
function createWarp($ip = "", $port = "")
{
    ...
    $baseV2rayng = "wireguard://" . urlencode(str_replace("%3D", "=", $private_key)) . "@{$ip}:{$port}?address=172.16.0.2%2F32%2C2606%3A4700%3A110%3A8f81%3Ad551%3Aa0%3A532e%3Aa2b3%2F128&reserved=" . urlencode($reserved) . "&publickey=" . urlencode(tr_replace("%3D", "=", $public_key)) . "&mtu=1280#{$hash}";
    return [
        $ip,
        $port,
        $private_key,
